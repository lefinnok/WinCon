...
from .WinConImg import renImgPath, renGif2Anim
...
class GifAnimFloat(AnimFloat):
    def __init__(self, path, sy, x, y, enable, invisible = False, freeForm = False, sx = 0, fps = 24, loop = False, playOnStart = False, contrast = 0):
        self.path = path
        self.freeForm = freeForm
        arys = renGif2Anim(path, sy, sx, freeForm, contrast)
        super().__init__(arys, x, y, enable, fps, loop, invisible, playOnStart)
...
def ren():
    screenX.singleFrameRender()

print(timeit(ren), number = 10)
